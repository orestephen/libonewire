...
int main(int argc, char** argv) {
  ...
  uint8_t known_buffer[] = {0x01, 0x12};

  assert(onewire_init(&onewire, test_device_get_dev(), onewire_timer_get_tick) ==
         ONEWIRE_ERROR_NONE);
  test_device_set_data(test_buffer, sizeof(test_buffer));
  assert(onewire_master_transmit(&onewire, known_buffer, sizeof(known_buffer),
                                 ONEWIRE_TIMEOUT_INFINITE) ==
         ONEWIRE_ERROR_NONE);
